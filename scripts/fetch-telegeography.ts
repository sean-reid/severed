...
async function main() {
	...
	writeFileSync(resolve(RAW_DIR, "cable-index.json"), JSON.stringify(cableIndex, null, 2));
	...
	writeFileSync(resolve(RAW_DIR, "cable-geo.json"), JSON.stringify(cableGeo, null, 2));
	console.log("  -> saved cable-geo.json");

	// 3. Fetch landing point geometries
	console.log("[3/4] Fetching landing point geometries...");
	const landingGeo = await fetchJson(`${BASE}/landing-point/landing-point-geo.json`);
	writeFileSync(resolve(RAW_DIR, "landing-point-geo.json"), JSON.stringify(landingGeo, null, 2));
	console.log("  -> saved landing-point-geo.json");

	// 4. Fetch individual cable details (rate limited)
	console.log(
		`[4/4] Fetching ${cableIndex.length} cable details (100ms delay between requests)...`,
	);
	...
	writeFileSync(resolve(RAW_DIR, "cable-details.json"), JSON.stringify(cableDetails, null, 2));
