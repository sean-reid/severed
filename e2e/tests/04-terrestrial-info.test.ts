...
export default async function test(ctx: TestContext) {
  ...
  const withUrls = await ctx.page.evaluate(() => {
    return fetch("/data/terrestrial.json")
      .then((r) => r.json())
      .then((data: Array<{ sourceUrl?: string }>) =>
        data.filter((e) => e.sourceUrl).length,
      );
  });
  ...
  const anyEdge = await ctx.page.evaluate(() => {
    return fetch("/data/terrestrial.json")
      .then((r) => r.json())
      .then((data: Array<{ id: string; from: string; to: string; capacityTbps: number; confidence: string; source: string }>) => {
        const e = data[0];
        return {
