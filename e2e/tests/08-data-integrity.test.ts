...
export default async function test(ctx: TestContext) {
  ...
  const cableStats = await ctx.page.evaluate(() =>
    fetch("/data/cables.json")
      .then((r) => r.json())
      .then((data: Array<{
        id: string; designCapacityTbps: number; capacityConfidence: string;
        capacitySource: string; sourceUrl?: string; segments: unknown[];
  ...
  ctx.assert(cableStats.withSegments >= 400, `Expected >=400 cables with segments, got ${cableStats.withSegments}`);
  ctx.assert(cableStats.verified >= 50, `Expected >=50 verified cables, got ${cableStats.verified}`);
  ctx.assert(cableStats.withSourceUrl >= 80, `Expected >=80 cables with sourceUrl, got ${cableStats.withSourceUrl}`);
  ...
  const terrStats = await ctx.page.evaluate(() =>
    fetch("/data/terrestrial.json")
      .then((r) => r.json())
      .then((data: Array<{
        id: string; from: string; to: string; capacityTbps: number;
        distanceKm: number; confidence: string; sourceUrl?: string;
      }>) => ({
        total: data.length,
  ...
  ctx.assert(terrStats.withSourceUrl >= 40, `Expected >=40 terrestrial edges with sourceUrl, got ${terrStats.withSourceUrl}`);
  ctx.assert(terrStats.maxDistance < 8000, `No terrestrial edge should exceed 8000km, got ${terrStats.maxDistance}`);
  ...
  ctx.assert(metroStats.validCoords === metroStats.total, `All metros should have valid coordinates`);
