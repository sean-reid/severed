...
import { createContext, type ViewportName } from "./context";
...
console.log(`\x1b[1mRunning ${testFiles.length} test suites across ${TEST_VIEWPORTS.length} viewports\x1b[0m\n`);
...
for (const file of testFiles) {
  ...
  for (const viewport of TEST_VIEWPORTS) {
    ...
    try {
      ...
      results.push({ name: suiteName, viewport, passed: false, error: msg, screenshot, durationMs: duration });
